...
public class PlatformGenerator : MonoBehaviour
{
    ...
    void Update()
    {
        if (transform.position.x < generationPoint.position.x)
        {
            ...
            {
                theCoinGenerator.SpawnCoins(new Vector3(transform.position.x, transform.position.y + 1f, 0));
            }
            ...
            if (Random.Range(0f, 100f) < randomSpikeTreshold)
            {
                ...
                Vector3 spikePositon = new Vector3(spikeXposition, 0.5f, 0f);

                newSpike.transform.position = transform.position + spikePositon;
